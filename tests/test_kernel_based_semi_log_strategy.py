...
class TestKernelBasedSemiLogStrategy:
    @pytest.fixture
    def kernel_based_semi_log_optimal_strategy(self) -> KernelBasedSemiLogStrategy:
        """
        Create a basic pytest fixture that will just provide a barebones implementation of Strategy
        """
        return KernelBasedSemiLogStrategy(num_assets=3)
    
    @pytest.fixture
    def fake_datasource(self) -> Dict[str, Union[np.array, KernelBasedSemiLogStrategy]]:
        """
    ...
    def test_constructor(self, kernel_based_semi_log_optimal_strategy: KernelBasedSemiLogStrategy) -> None:
        """
        Check that the constructor initializes the strategy object as expected.
        """
        strat = kernel_based_semi_log_optimal_strategy
    ...
    def test_update_with_price_relatives(self, kernel_based_semi_log_optimal_strategy: KernelBasedSemiLogStrategy, fake_datasource: Dict[str, Union[np.array, KernelBasedDataSource]]) -> None:
        """
        ...
        assert isclose(strat.weights[0],0.3300713522)
        assert isclose(strat.weights[1],0.3337933276)
        assert isclose(strat.weights[2],0.33613532020)
